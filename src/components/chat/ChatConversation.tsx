...
import { Bot, Send, User as UserIcon, Wrench } from "lucide-react";
  ...
  React.useEffect(() => {
    ...
    const loadCatalog = async () => {
      try {
        ...
        setCommandsByKey(Object.fromEntries(json.commands.map((entry) => [entry.key, entry])));
      } catch {
    ...
    try {
      ...
      if (!res.ok) {
        addToolOutput({
          ...
          state: "output-error",
          errorText: String(
            (output as { detail?: string; error?: string })?.detail ||
          ...
          {messages.length === 0 && (
            <>
              <div className="flex items-start gap-2">
                ...
                <div className="max-w-[85%] rounded-lg bg-muted px-3 py-2 text-sm">{greetingMsg}</div>
              </div>
          ...
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={cn("flex items-start gap-2", m.role === "user" ? "flex-row-reverse" : "")}>
                {m.role === "user" ? (
                  <UserIcon className="mt-0.5 h-5 w-5 shrink-0 text-muted-foreground" />
                ) : (
                  <Bot className="mt-0.5 h-5 w-5 shrink-0 text-accent" />
                )}
                  ...
                  {toolParts.map((p, i) => {
                    ...
                    if (name === "execute_portal_action" && p.state === "input-available") {
                      ...
                      return (
                        <div
                          key={callId}
                          className="space-y-2 rounded-md border border-border bg-secondary/50 p-3 text-xs text-muted-foreground"
                        >
                          ...
                          {isDestructive && (
                            <Input
                              ...
                              onChange={(e) => setConfirmTextByCall((prev) => ({ ...prev, [callId]: e.target.value }))}
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              disabled={
                                !!runningCalls[callId] ||
                                (isDestructive &&
                                  (confirmTextByCall[callId] || "").trim().toUpperCase() !== "CONFIRM")
                              }
                            >
                    ...
                    if (name === "execute_portal_action" && p.state === "output-available") {
                      return (
                        <div
                          key={callId}
                          className="flex items-center gap-2 rounded-md border border-border bg-secondary/50 px-3 py-2 text-xs text-muted-foreground"
                        >
                          <Wrench className="h-3.5 w-3.5" />
                          Acción ejecutada.
                        </div>
